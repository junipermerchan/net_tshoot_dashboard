...
class WebApp {
    ...
    renderCurrentStep() {
        ...
        const rawCmds = this.getStepCommands(step, this.activeVendor);
...
1/4/1      Registered  ZTEGC1A2B3D4  Default  Up`;
    ...
    getStepCommands(step, vendor) {
        if (!step) return [];
        const source = (this.activeIpVersion === 'ipv6' && step.commands_ipv6) ? step.commands_ipv6 : step.commands;
    ...
    filterCommandsByIpVersion(cmds, version) {
        ...
        return cmds.filter(cmd => {
            ...
            const isIpv6Cmd = clean.includes('ipv6') || 
                               ...
                               clean.includes('fe80::') ||
                               clean.includes('address-family ipv6') ||
                               clean.includes('address family ipv6') ||
                               clean.includes('ipv6-family') ||
                               clean.includes('info6') ||
                               clean.includes('route-static ipv6');
            ...
            const isIpv4Cmd = clean.includes('ip route') || 
                               ...
                               clean.includes('ip interface') || 
                               clean.includes('ip neighbor') || 
                               clean.includes('arp ') || clean.includes(' arp ') || clean.startsWith('arp ') ||
                               clean.includes('igmp') ||
                               clean.includes('/ip route') ||
                               clean.includes('192.168.') ||
                               clean.includes('10.10.') ||
                               clean.includes('address-family ipv4') ||
                               clean.includes('address family ipv4') ||
                               clean.includes('ipv4-family') ||
                               clean.includes('router info bgp') ||
                               (clean.includes('show route') && !clean.includes('inet6') && !clean.includes('ipv6') && !clean.includes('vrf')) ||
                               (clean.includes('display routing-table') && !clean.includes('ipv6')) ||
                               (clean.includes('routing-table') && !clean.includes('info6') && !clean.includes('ipv6')) ||
                               (clean.includes('router info') && !clean.includes('info6') && !clean.includes('ipv6') && !clean.includes('ospf3') && !clean.includes('ospfv3')) ||
                               (clean.includes('ospf') && !clean.includes('ospf3') && !clean.includes('ospfv3') && !clean.includes('ospf v3') && !clean.includes('ospf6')) ||
                               (clean.includes('bgp') && !clean.includes('ipv6') && !clean.includes('vpnv6') && !clean.includes('address-family ipv6') && !clean.includes('address family ipv6') && !clean.includes('ipv6-family'));
    ...
    exportRemediationRunbook() {
        if (!this.notesLog || this.notesLog.length === 0) {
            const alertMsg = this.currentLang === 'es' ? 
                'No hay notas ni soluciones registradas para generar el Runbook.' : 
                'No notes or fixes logged to generate the Runbook.';
            alert(alertMsg);
            return;
        }
        ...
        this.notesLog.forEach(n => {
            const note = n.note || '';
            if (note.includes('Comandos de reparación sugeridos:') || note.includes('$ ')) {
                fixLines.push(`# === ${n.title} ===`);
                note.split('\n').forEach(line => {
                    if (line.trim().startsWith('$ ')) {
                        fixLines.push(line.trim().replace('$ ', ''));
        ...
        if (fixLines.length === 0) {
            const alertMsg = this.currentLang === 'es' ? 
                'No se encontraron comandos de remediación ($) en la bitácora activa.' : 
                'No remediation commands ($) found in the active session log.';
            alert(alertMsg);
            return;
        }

        const content = "# RUNBOOK DE REMEDIACIÓN Y COMANDOS FIX\n# Generado por Net Troubleshoot Dashboard\n\n" + fixLines.join('\n');
        const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `runbook_remediacion_${new Date().toISOString().slice(0,10)}.txt`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
    ...
    renderTheoryConcepts() {
        ...
        // 1. Try to look up IPv6 specific concepts if IPv6 mode is active
        if (this.activeIpVersion === 'ipv6') {
            concepts = this.data.TECH_CONCEPTS[techKey + '_ipv6'];
            if (!concepts && techKey && techKey.endsWith('_config')) {
                const baseKey = techKey.replace('_config', '');
                concepts = this.data.TECH_CONCEPTS[baseKey + '_ipv6'];
            }
        }
        
        // 2. Fallback to standard concepts
        if (!concepts) {
            concepts = this.data.TECH_CONCEPTS[techKey];
            if (!concepts && techKey && techKey.endsWith('_config')) {
                const baseKey = techKey.replace('_config', '');
...
`
    ...
    openMagicDiagnosticView() {
        this.showView('magic-diagnostic');
        document.getElementById('diag-input-text').value = '';
        document.getElementById('diagnostic-result-card').classList.add('hidden');
        this.activeDiagReport = null;
    }

    clearMagicDiagnosticInput() {
        document.getElementById('diag-input-text').value = '';
        document.getElementById('diagnostic-result-card').classList.add('hidden');
        this.activeDiagReport = null;
    }
    ...
    detectVendorJS(text) {
        ...
        if (lower.includes("display set") || lower.includes("show configuration protocols") || lower.includes("show configuration interfaces") || lower.includes("commit |")) {
            return "juniper";
        }
        if (/\b(ge|xe|et)-\d+\/\d+\/\d+/.test(lower) || lower.includes("juniper networks")) {
            return "juniper";
        }
        if (lower.includes("rp/0/rsp0/cpu0") || lower.includes("show running-config formal") || lower.includes("ios-xr")) {
            return "cisco_iosxr";
        }
        if (lower.includes("show ip interface brief") || lower.includes("show ip route vrf") || lower.includes("show running-config")) {
            if (!lower.includes("ios-xr")) {
                return "cisco_iosxe";
            }
        }
        if (lower.includes("arista eos") || lower.includes("show vxlan vtep") || lower.includes("show bgp evpn") || (lower.includes("port-channel") && lower.includes("vxlan"))) {
            return "arista";
        }
        if (lower.includes("configure router") || lower.includes("show service id") || lower.includes("bof") || lower.includes("show service service-using")) {
            return "nokia";
        }
        if (lower.includes("/ip address") || lower.includes("/interface vlan") || lower.includes("/mpls ldp") || lower.includes("routeros")) {
            return "mikrotik";
        }
        if (lower.includes("config system interface") || lower.includes("config router static") || lower.includes("diagnose sys sdwan") || lower.includes("fortigate")) {
            return "fortinet";
        }
        if (lower.includes("display ip routing-table") || lower.includes("display current-configuration") || (lower.includes("sysname") && lower.includes("olt"))) {
            return "huawei";
        }
        if (lower.includes("interface gpon-onu_") || lower.includes("pon-onu-mng") || lower.includes("zteg-")) {
            return "zte";
        }
        if (lower.includes("gpon-olt 1") || lower.includes("show ethernet service")) {
            return "adtran";
        }
        if (lower.includes("root@linux") || lower.includes("ip route show") || lower.includes("iptables -") || lower.includes("tcpdump -")) {
            return "linux";
        }
        return null;
    }

    runMagicDiagnosticAnalysis() {
        const text = document.getElementById('diag-input-text').value.trim();
        if (!text) {
            alert(this.currentLang === 'es' ? "Por favor pegue algún texto para analizar." : "Please paste some text to analyze.");
            return;
        }

        const selectedVendorVal = document.getElementById('diag-vendor-select').value;
        const detectedVendor = this.detectVendorJS(text);
        const vendor = selectedVendorVal === 'auto' ? (detectedVendor || 'cisco_iosxe') : selectedVendorVal;
        ...
        const VENDORS_MAP = {
            ...
            "huawei": "Huawei VRP",
            "zte": "ZTE GPON OLT",
            "adtran": "ADTRAN TA5000",
            "linux": "Linux"
        };
        const vendorLabel = VENDORS_MAP[vendor] || vendor;

        const lines = text.split("\n");
        let report = null;

        // 1. OSPF MTU
        let ospfMtuMatch = false;
        let anomalousLines = [];
        for (let line of lines) {
            if (line.includes("EXSTART") || line.includes("Exchange") || line.includes("Exchange/ExStart") || line.includes("Exstart/Exchange") || 
                line.toLowerCase().includes("mtu mismatch") || line.toLowerCase().includes("different mtu") || line.toLowerCase().includes("mismatch mtu")) {
                ospfMtuMatch = true;
                anomalousLines.push(line);
            }
        }
        ...
        if (ospfMtuMatch) {
            report = {
                ...
                architectural_cause: "Durante la fase de sincronización de la Base de Datos (LSDB), los vecinos OSPF intercambian paquetes Database Description (DBD). El RFC 2328 estipula que un router debe ignorar los paquetes DBD si el MTU anunciado por el vecino es mayor que su propio MTU de interfaz. Esto congela la adyacencia en estado EXSTART o EXCHANGE indefinidamente, previniendo el cálculo de rutas SPF y aislando el tráfico de tránsito.",
                acceptance_criteria: "El comando de estado de adyacencia debe reportar el estado 'FULL' en ambos extremos del enlace, y las MTUs configuradas físicamente deben ser consistentes e idénticas.",
                anomalous_lines: anomalousLines,
                solutions: {
                    "cisco_iosxe": [
        ...
        // 2. BGP MD5
        if (!report) {
            let bgpMd5Match = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.includes("BADAUTH") || line.includes("MD5 digest") || line.includes("MD5 signature failed") || 
                    line.includes("MD5 signature mismatch") || line.includes("No MD5 digest") || line.includes("TCP MD5 Signature Option") ||
                    (line.includes("TCP: Connection reset by peer") && (line.includes("BGP") || line.includes("179")))) {
                    bgpMd5Match = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (bgpMd5Match) {
                report = {
                    ...
                    architectural_cause: "Para proteger las sesiones BGP de ataques de inyección y suplantación de identidad TCP, se utiliza la opción de firma MD5 de TCP (RFC 2385). Si las contraseñas pre-compartidas difieren en un solo carácter o si un extremo no tiene configurada la contraseña, el socket TCP rechaza inmediatamente los segmentos SYN o los descarta en silencio. Esto impide establecer el puerto 179 y la sesión BGP se mantiene alternando entre los estados 'Active' e 'Idle'.",
                    acceptance_criteria: "La sesión BGP debe establecer el estado 'Established' de forma permanente, y no deben registrarse logs de 'TCP-6-BADAUTH' o descartes de firmas de seguridad en la consola.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        if (!report) {
            ...
            anomalousLines = [];
            for (let line of lines) {
                if (line.toLowerCase().includes("route target mismatch") || line.toLowerCase().includes("no import route target") || 
                    line.toLowerCase().includes("import rt mismatch") || line.toLowerCase().includes("discarded due to rt") || 
                    line.toLowerCase().includes("rt 0:0") || line.toLowerCase().includes("rt mismatch") ||
                    (line.includes("received") && line.includes("imported") && line.includes("0 imported"))) {
                    rtMismatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (rtMismatch) {
                report = {
                    ...
                    architectural_cause: "En arquitecturas MPLS L3VPN, los Route Targets (RT) son comunidades extendidas de BGP que controlan la importación y exportación de rutas entre la tabla global de VPNv4 (bgp.l3vpn.0) y las instancias de enrutamiento VRF locales. Si el PE transmisor exporta con un RT (ej. 65000:100) pero el PE receptor no tiene configurada esa misma comunidad en su política de importación de la VRF, las rutas son descartadas silenciosamente al recibirse. BGP muestra la sesión Up, pero el cliente experimenta aislamiento completo.",
                    acceptance_criteria: "La tabla de ruteo de la VRF destino debe poblarse con los prefijos anunciados por el PE remoto, lo cual se confirma asegurando que export-rt en el origen sea idéntico a import-rt en el receptor.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
                        ...
                        "juniper": [
                            "Revisar el bloque `route-distinguisher` and `vrf-target` de la instancia de ruteo.",
        ...
        // 4. LDP Transport Address Unreachable
        if (!report) {
            let ldpTransportMatch = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.includes("transport-address") || line.includes("transport address connection failed") || 
                    line.includes("LDP session could not be established") || (line.includes("LDP") && line.includes("NonExistent"))) {
                    ldpTransportMatch = true;
                    anomalousLines.push(line);
                }
            }

            if (ldpTransportMatch) {
                report = {
                    matched: true,
                    ...
                    architectural_cause: "LDP utiliza Hellos UDP (puerto 646) para descubrir vecinos directamente conectados. Sin embargo, para intercambiar etiquetas (bindings), establece una sesión TCP de Capa 4 contra la 'Transport Address' anunciada en los Hellos (típicamente la IP de la Loopback). Si la Loopback del vecino no está publicada en el IGP (OSPF/IS-IS) o si hay filtros/ACLs bloqueando TCP 646, la sesión TCP no se establece (se queda en NonExistent o Initialized), rompiendo la señalización de etiquetas de transporte.",
                    acceptance_criteria: "La sesión LDP debe pasar a estado 'Operational' o 'Established' y la ruta IGP hacia la loopback del vecino debe ser alcanzable con una máscara de host /32 exacta.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 5. LACP Port Suspended
        if (!report) {
            let lacpSuspended = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.includes("PORT_SUSPENDED") || line.includes("suspended") || line.includes("LACP-3-PORT_SUSPENDED") || 
                    line.includes("individual") || line.includes("Suspended") || line.includes("Individual") ||
                    (line.toLowerCase().includes("lacp") && line.toLowerCase().includes("down"))) {
                    lacpSuspended = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (lacpSuspended) {
                report = {
                    ...
                    architectural_cause: "Cuando un puerto se configura con LACP dinámico (modo active), espera intercambiar PDUs de LACP con el extremo remoto para formar un canal lógico (Bundle/Port-Channel). Si el extremo remoto tiene configurado el EtherChannel en modo estático (mode on), no envía PDUs de LACP. En este escenario, para evitar bucles o inconsistencias de Capa 2, LACP suspende los puertos locales o los marca en estado 'Individual', deshabilitando el reenvío de datos.",
                    acceptance_criteria: "El estado del Port-Channel debe reportarse en Up y los miembros físicos deben mostrar el estado 'Bundle' o 'Active' indicando intercambio de control exitoso.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 6. STP Root Guard Blocked
        if (!report) {
            let stpRootGuard = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.includes("ROOT-GUARD-BLOCK") || line.includes("root guard") || line.includes("Root Port blocked") || 
                    line.includes("port blocked by Root Guard") || line.toLowerCase().includes("loop guard") || line.toLowerCase().includes("bpdu-guard")) {
                    stpRootGuard = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (stpRootGuard) {
                report = {
                    ...
                    architectural_cause: "Root Guard protege la topología activa de STP asegurando que la ubicación elegida para el Root Bridge principal no cambie de forma no autorizada. Si un puerto con Root Guard habilitado recibe un BPDU superior (que anuncia un Bridge ID con menor prioridad/MAC), el puerto transita inmediatamente al estado 'root-inconsistent' (bloqueado). Esto previene que un switch externo erróneo o no autorizado tome control de la topología lógica de la red.",
                    acceptance_criteria: "La prioridad de STP en los switches principales de Core debe configurarse explícitamente a un valor muy bajo (ej. 4096) para evitar elecciones accidentales, restaurando el puerto bloqueado.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 7. GPON Low Optical Power
        if (!report) {
            let gponOptPower = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.includes("dBm") && (line.includes("-28.") || line.includes("-29.") || line.includes("-30.") || line.includes("-31.") || 
                    line.includes("-32.") || line.includes("-33.") || line.includes("-34.") || line.includes("-35.") || 
                    line.toLowerCase().includes("low") || line.toLowerCase().includes("critical") || line.toLowerCase().includes("too low"))) {
                    gponOptPower = true;
                    anomalousLines.push(line);
                } else if (line.toLowerCase().includes("optical power") && (line.toLowerCase().includes("low") || line.toLowerCase().includes("alarm"))) {
                    gponOptPower = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (gponOptPower) {
                report = {
                    ...
                    architectural_cause: "El estándar ITU-T G.984 especifica que la sensibilidad óptica de recepción (RX) en la ONU/ONT debe mantenerse dentro del rango de -15 dBm a -27 dBm. Si el nivel cae por debajo de -27 dBm (por ejemplo, a -30 dBm o menos) debido a conectores sucios, empalmes defectuosos, atenuaciones en splitters o curvaturas críticas en la fibra (macrobends), el transceptor sufre pérdida de tramas FEC, causando desconexiones intermitentes del cliente (flapping ONT) o pérdida total del servicio.",
                    acceptance_criteria: "La potencia de recepción medida en la ONT debe registrarse entre -15 dBm y -25 dBm para garantizar la estabilidad del servicio.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "huawei": [
        ...
        // 8. Fortinet SD-WAN SLA Fail
        if (!report) {
            let fortinetSdwanSla = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.toLowerCase().includes("sdwan") && (line.toLowerCase().includes("dead") || line.toLowerCase().includes("sla failed") || line.toLowerCase().includes("latency exceeded") || line.toLowerCase().includes("packet loss"))) {
                    fortinetSdwanSla = true;
                    anomalousLines.push(line);
                } else if (line.toLowerCase().includes("health-check") && (line.toLowerCase().includes("failed") || line.toLowerCase().includes("dead"))) {
                    fortinetSdwanSla = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (fortinetSdwanSla) {
                report = {
                    ...
                    architectural_cause: "FortiOS utiliza Health-Checks activos (pings, queries DNS o peticiones HTTP) para monitorear el desempeño de cada miembro del SD-WAN (latencia, jitter y pérdida de paquetes). Si un miembro excede los umbrales configurados para un SLA (por ejemplo, pérdida de paquetes > 5% o latencia > 150 ms) o deja de responder, el motor de SD-WAN marca el enlace como DEAD. Como consecuencia, el tráfico de producción se redirige a otros miembros saludables que cumplan con la regla de negocio.",
                    acceptance_criteria: "El estado del SLA debe reportar 'ALIVE' para todos los miembros, y las métricas de red deben mantenerse estables por debajo del umbral objetivo de la política de calidad de servicio.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "fortinet": [
        ...
        // 9. EVPN ESI Inconsistency
        if (!report) {
            let evpnEsiMatch = false;
            anomalousLines = [];
            for (let line of lines) {
                if (line.includes("ESI") && (line.includes("mismatch") || line.includes("inconsistency") || line.includes("DF election failed") || line.includes("Designated Forwarder") || line.includes("split-brain"))) {
                    evpnEsiMatch = true;
                    anomalousLines.push(line);
                } else if (line.toLowerCase().includes("esi") && line.toLowerCase().includes("inconsistent")) {
                    evpnEsiMatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (evpnEsiMatch) {
                report = {
                    ...
                    architectural_cause: "En multihoming activo-activo de EVPN, dos o más PEs se conectan al mismo switch de cliente mediante un Link Aggregation Group (LAG). Para evitar bucles y asegurar que el tráfico BUM se replique de forma controlada, los PEs deben configurar el mismo ESI de 10 bytes. Si hay un mismatch de ESI, el proceso de elección del Designated Forwarder (DF) se rompe, lo cual provoca problemas de duplicidad de tramas, loops lógicos o descarte total del tráfico.",
                    acceptance_criteria: "El ESI configurado en el LAG del cliente debe ser idéntico en ambos PEs redundantes, y la elección del DF debe completarse con un único Designated Forwarder activo por VLAN.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxr": [
        ...
        if (!report) {
            ...
            for (let line of lines) {
                ...
                if (l.includes("bfd") && ["down", "admindown", "timer mismatch", "detect timer expired", "detect-multiplier", "minimum-interval"].some(x => l.includes(x))) {
                    bfdMatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (bfdMatch) {
                report = {
                    ...
                    architectural_cause: "BFD proporciona detección ultrarrápida de fallas de enlace sub-segundo para protocolos de enrutamiento (BGP, OSPF, IS-IS). Si los temporizadores negociados no son compatibles o el multiplicador de detección expira (por congestión o descarte UDP 3784/3785), BFD colapsa la sesión, provocando la caída en cadena de las adyacencias asociadas.",
                    acceptance_criteria: "La sesión BFD debe reportarse en estado 'Up' con un tiempo de respuesta de Keepalive estable en ambas direcciones.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        if (!report) {
            ...
            for (let line of lines) {
                let l = line.toLowerCase();
                if (l.includes("bgp") && ["as_path", "as loop", "deny local as", "as_override", "loop detected in as_path", "rejecting route due to local as"].some(x => l.includes(x))) {
                    bgpAsLoop = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (bgpAsLoop) {
                report = {
                    ...
                    architectural_cause: "En topologías MPLS L3VPN donde un cliente utiliza el mismo ASN en múltiples sitios remotos, la prevención de bucles de BGP rechaza silenciosamente los prefijos que contienen el ASN propio. Sin `as-override` en el PE (que reemplaza el AS del cliente por el AS del SP), las rutas no se instalarán en la VRF del sitio remoto.",
                    acceptance_criteria: "Los prefijos del sitio remoto deben ser visibles e instalados en la FIB de la VRF con el ASN sustituido correctamente.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 12. IS-IS Adjacency Mismatch
        if (!report) {
            let isisMatch = false;
            anomalousLines = [];
            for (let line of lines) {
                let l = line.toLowerCase();
                if ((l.includes("isis") || l.includes("is-is")) && ["area mismatch", "system id collision", "lsp mtu mismatch", "init state", "rejected iih"].some(x => l.includes(x))) {
                    isisMatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (isisMatch) {
                report = {
                    ...
                    architectural_cause: "Para establecer adyacencias Level-1 en IS-IS, sus Area IDs deben coincidir de forma idéntica. Si difieren, la adyacencia queda en 'Init'. Si el tamaño del LSP supera el MTU físico, el flooding de la base de datos colapsa.",
                    acceptance_criteria: "El comando `show isis neighbors` debe mostrar el estado 'Up' o 'L1L2' en ambos extremos del enlace.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 13. QoS Output Queue Drops
        if (!report) {
            let qosDrops = false;
            anomalousLines = [];
            for (let line of lines) {
                let l = line.toLowerCase();
                if ((l.includes("qos") || l.includes("queue") || l.includes("interface")) && ["output drop", "tail drop", "wred drop", "threshold exceed", "buffer exhaustion", "overrun"].some(x => l.includes(x))) {
                    qosDrops = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (qosDrops) {
                report = {
                    ...
                    architectural_cause: "Los descartes en colas de salida ocurren cuando la tasa de entrada ráfaga supera el clock rate de la interfaz de salida o el shaper del policy-map, saturando los buffers del hardware.",
                    acceptance_criteria: "Los contadores de 'output drops' y 'tail drops' deben permanecer estables en 0, y el tráfico de prioridad (EF) no debe sufrir pérdidas.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 14. DHCP Snooping Untrusted Port
        if (!report) {
            let dhcpSnoop = false;
            anomalousLines = [];
            for (let line of lines) {
                let l = line.toLowerCase();
                if (l.includes("dhcp") && ["snooping", "option 82", "untrusted port", "dhcp_snooping_untrusted_port", "rate limit exceeded", "binding database missing"].some(x => l.includes(x))) {
                    dhcpSnoop = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (dhcpSnoop) {
                report = {
                    ...
                    architectural_cause: "DHCP Snooping descarta paquetes DHCP Offer/ACK entrantes en puertos no confiables para evitar servidores DHCP rogue o incongruencias con Option 82.",
                    acceptance_criteria: "La interfaz de enlace ascendente (Uplink) hacia el servidor DHCP debe estar explícitamente configurada en modo 'trust' y los clientes deben recibir su IP sin descartes.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 15. DMVPN IPsec Proposal Mismatch
        if (!report) {
            let ipsecMatch = false;
            anomalousLines = [];
            for (let line of lines) {
                let l = line.toLowerCase();
                if (["dmvpn", "ipsec", "ike", "isakmp"].some(x => l.includes(x)) && ["no_proposal_chosen", "phase 1 failed", "phase 2 failed", "crypto map mismatch", "sa proposal mismatch", "mismatch transform set"].some(y => l.includes(y))) {
                    ipsecMatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (ipsecMatch) {
                report = {
                    ...
                    architectural_cause: "Ambas partes deben acordar exactamente los mismos parámetros criptográficos en Fase 1 (Cifrado, Hash, DH group) y Fase 2 (Transform Set). Discrepancias causan rechazo de SA.",
                    acceptance_criteria: "La consulta `show crypto ipsec sa` debe reportar estados 'ACTIVE' en las SAs entrantes y salientes.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 16. VXLAN VNI Mismatch
        if (!report) {
            let vxlanMatch = false;
            anomalousLines = [];
            for (let line of lines) {
                let l = line.toLowerCase();
                if (["vxlan", "vni"].some(x => l.includes(x)) && ["multicast group mismatch", "flood list empty", "vni not mapped", "vni down", "nve interface down"].some(y => l.includes(y))) {
                    vxlanMatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (vxlanMatch) {
                report = {
                    ...
                    architectural_cause: "En VXLAN data plane, la VNI debe mapearse al mismo grupo multicast o VTEP peer list para replicar tráfico BUM. Desajustes impiden el establecimiento del túnel NVE.",
                    acceptance_criteria: "La interfaz NVE debe estar en estado 'Up', mapeando la VNI objetivo al grupo multicast o lista VTEP activa.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        // 17. PBR Next-Hop Unreachable
        if (!report) {
            let pbrMatch = false;
            anomalousLines = [];
            for (let line of lines) {
                let l = line.toLowerCase();
                if (["pbr", "policy routing", "policy-map"].some(x => l.includes(x)) && ["next-hop unreachable", "track failed", "pbr recursive lookup failed", "fallback to default"].some(y => l.includes(y))) {
                    pbrMatch = true;
                    anomalousLines.push(line);
                }
            }
            ...
            if (pbrMatch) {
                report = {
                    ...
                    architectural_cause: "Si la IP de `set ip next-hop` no está en la FIB o si el objeto IP SLA (track) cae, PBR cae al enrutamiento RIB estándar o descarta paquetes.",
                    acceptance_criteria: "El objeto de rastreo (track object) debe reportar estado 'UP' y la IP del nexthop debe resolverse en la RIB activa.",
                    anomalous_lines: anomalousLines,
                    solutions: {
                        "cisco_iosxe": [
        ...
        if (!report) {
            let genericErrors = [];
            for (let line of lines) {
                if (["error", "fail", "down", "mismatch", "conflict", "shutdown", "drop", "discard", "collision"].some(x => line.toLowerCase().includes(x))) {
                    genericErrors.push(line);
                }
            }
            ...
            if (genericErrors.length > 0) {
                report = {
                    ...
                    architectural_cause: "El analizador ha escaneado la salida pegada y ha detectado líneas que indican estados caídos (Down), desajustes (Mismatch), caídas de enlaces o colisiones físicas. Estos problemas suelen estar asociados a fallas en la capa física, errores de configuración local, o desalineaciones con el equipo adyacente.",
                    acceptance_criteria: "Todos los estados de puertos deben reportar Up/Up, sin contadores de error incrementándose.",
                    anomalous_lines: genericErrors.slice(0, 8),
                    solutions: {
                        "cisco_iosxe": [
        ...
        if (!report) {
            report = {
                ...
                severity: "Baja",
                rfc_reference: "N/A",
                architectural_cause: "El analizador no ha identificado ninguna firma de error crítica ni palabras clave de fallas graves en el texto provisto. El estado de la configuración o comando parece saludable o no está dentro de la base de datos de firmas conocidas.",
        ...
        if (report.severity === 'Crítica') {
            resultCard.style.borderLeftColor = '#ef4444'; // Red
            document.getElementById('diag-res-tech-badge').style.backgroundColor = '#ef4444';
        } else if (report.severity === 'Alta') {
            resultCard.style.borderLeftColor = '#f97316'; // Orange
            document.getElementById('diag-res-tech-badge').style.backgroundColor = '#f97316';
        } else {
            resultCard.style.borderLeftColor = '#3b82f6'; // Blue
            document.getElementById('diag-res-tech-badge').style.backgroundColor = '#3b82f6';
        }

        document.getElementById('diag-res-tech-badge').innerText = report.technology.split(" ")[0];
        document.getElementById('diag-res-title').innerText = report.problem_title;
        document.getElementById('diag-res-meta').innerText = `Severidad: ${report.severity} | RFC: ${report.rfc_reference}`;
        document.getElementById('diag-res-cause').innerText = report.architectural_cause;
        document.getElementById('diag-res-acceptance').innerText = report.acceptance_criteria;

        // Anomalous lines
        const anomalousSection = document.getElementById('diag-res-anomalous-section');
        if (report.anomalous_lines && report.anomalous_lines.length > 0) {
            anomalousSection.classList.remove('hidden');
            document.getElementById('diag-res-anomalous-code').innerText = report.anomalous_lines.join("\n");
        } else {
            anomalousSection.classList.add('hidden');
        }

        // Solutions Multi-vendor tabs
        const tabsList = document.getElementById('diag-sol-tabs-list');
        const panelContent = document.getElementById('diag-sol-panel-content');
        tabsList.innerHTML = '';
        panelContent.innerHTML = '';

        const solKeys = Object.keys(report.solutions || {});
        if (solKeys.length > 0) {
            solKeys.forEach((key, idx) => {
                const vendorName = VENDORS_MAP[key] || key.toUpperCase();
                const btn = document.createElement('button');
                btn.className = `concept-tab-btn \${idx === 0 ? 'active' : ''}`;
                btn.style.padding = '6px 12px';
                btn.style.fontSize = '0.85em';
                btn.style.cursor = 'pointer';
                btn.innerText = vendorName;
                btn.onclick = (e) => this.selectDiagnosticSolTab(key, e);
                tabsList.appendChild(btn);

                if (idx === 0) {
                    const codeBlockText = report.solutions[key].map(c => `$ \${c}`).join("\n");
                    panelContent.innerHTML = `
                        <pre style="background: rgba(15,23,42,0.6); padding:10px 15px; border-radius:6px; font-family:'Fira Code', monospace; color:#34d399; font-size:0.9em; margin:0; overflow-x:auto;"><code>\${codeBlockText}</code></pre>
                        <button class="btn-copy" style="margin-top:10px; cursor:pointer;" onclick="navigator.clipboard.writeText(app.activeDiagReport.solutions['\${key}'].join('\\n')).then(() => alert('Comandos copiados al portapapeles!'))">
                            <i data-lucide="copy" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:4px;"></i>
            ...
            panelContent.innerHTML = `<p style="opacity:0.7; font-size:0.9em; margin:0;">\${this.currentLang === 'es' ? 'No hay comandos de configuración preestablecidos para este problema.' : 'No configuration commands pre-established for this issue.'}</p>`;
        }
    ...
    selectDiagnosticSolTab(vendorKey, event) {
        // Toggle active tabs
        event.target.parentNode.querySelectorAll('.concept-tab-btn').forEach(btn => btn.classList.remove('active'));
        event.target.classList.add('active');

        const panelContent = document.getElementById('diag-sol-panel-content');
        const codeBlockText = this.activeDiagReport.solutions[vendorKey].map(c => `$ \${c}`).join("\n");
        panelContent.innerHTML = `
            <pre style="background: rgba(15,23,42,0.6); padding:10px 15px; border-radius:6px; font-family:'Fira Code', monospace; color:#34d399; font-size:0.9em; margin:0; overflow-x:auto;"><code>\${codeBlockText}</code></pre>
            <button class="btn-copy" style="margin-top:10px; cursor:pointer;" onclick="navigator.clipboard.writeText(app.activeDiagReport.solutions['\${vendorKey}'].join('\\n')).then(() => alert('Comandos copiados al portapapeles!'))">
                <i data-lucide="copy" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:4px;"></i>
                <span>Copiar Comandos</span>
            </button>
        `;
    ...
    saveMagicDiagnosticToNotes() {
        if (!this.activeDiagReport) return;
        
        const report = this.activeDiagReport;
        let noteText = `PROBLEMA DETECTADO: \${report.problem_title}\n` +
                       `Tecnología: \${report.technology} | Severidad: \${report.severity}\n` +
                       `RFC: \${report.rfc_reference}\n` +
                       `Causa Raíz: \${report.architectural_cause}\n` +
                       `Criterio de Aceptación: \${report.acceptance_criteria}`;
        
        if (report.solutions) {
            noteText += `\nComandos de reparación sugeridos:`;
            Object.keys(report.solutions).forEach(vk => {
                noteText += `\n  - \${vk.toUpperCase()}:`;
                report.solutions[vk].forEach(c => {
                    noteText += `\n    $ \${c}`;
                });
            });
        }
        ...
        this.notes.push({
            techKey: 'diagnostico_magico',
            techName: this.currentLang === 'es' ? 'Analizador Inteligente' : 'Intelligent Analyzer',
            stepKey: 'diagnostico_magico',
            stepTitle: this.currentLang === 'es' ? `Diagnóstico: \${report.problem_title}` : `Diagnosis: \${report.problem_title}`,
            note: noteText,
            timestamp: timestamp
        });

        this.renderSidebarNotes();
        this.updateConfidenceIndicator(15); // Add confidence

        alert(this.currentLang === 'es' ? "Diagnóstico guardado con éxito en la bitácora!" : "Diagnosis successfully saved to the log!");
    }
}
